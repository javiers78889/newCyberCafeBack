...
export const EntregaPaquetes = async (user: Email) => {

    console.log(user.correo)
    const mailOptions = {
        from: process.env.EMAIL, // Remitente
        to: `${user.correo}`, // Destinatario (cambia esta dirección)
        subject: `El Paquete Con N° DE FACTURA FS-${user.id} Ha Sido Entregado!`, // Asunto
        text: `No comparta con nadie`, // Cuerpo en texto plano
        html: `<h5 style="font-size: 22px">Hola ${user.usuario}. Ha sido ${user.status} un paquete!</h5>
        </br>
        ...
        <tr>
            ...
            <tr>
                <td style="padding: 8px; font-weight: bold;">Chame</td>
            </tr>
            <tr>
    ...
    <thead>
        <tr style="background-color: red; color: white;">
            ...
            <th style="padding: 10px; border: 1px solid white;">ESTADO</th>
        </tr>
    </thead>
    <tbody>
...
<h1>TOTAL CANCELADO $${user.precio}</h1>
</br>
